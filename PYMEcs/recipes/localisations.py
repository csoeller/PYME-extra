...
class DriftCorrComet(ModuleBase):
    ...
    outputDrift = Output('drift_estimate')

    segmentation_mode = Enum(["locs_per_window","frames_per_window","time_window"]) # we make locs per window the default
    segmentation_var = Int(60)
    max_locs_per_segment = Int(200)
    target_sigma_nm = Float(1.5)
    ...
    def run(self, inputName):
        ...
        return dict(outputName=out, outputDrift=driftds)
...
class OrigamiSiteTrack(ModuleBase):
    ...
    outputDrift : name of tabular output that will contain
        estimated drift time course
    ...
    outputDrift = Output('drift_estimate_sites')
    ...
    def run(self, inputClusters, inputSites,inputAllPoints=None):
        ...
        tim = np.arange(1e-3*t.min(),1e-3*t.max(),1.0)
        drift_dict = dict(tim=tim,t=1e3*tim,x=c_xsite(1e3*tim),y=c_ysite(1e3*tim))
        if has_z:
            drift_dict['z'] = c_zsite(1e3*tim)
        from PYME.IO.tabular import DictSource
        drift_ds = DictSource(drift_dict)
        return {'outputName': mapped_ds, 'outputAllPoints' : mapped_ap , 'outputDrift' : drift_ds}
