    ...
    for mod in pipeline.recipe.modules:
        if isinstance(mod,OrigamiSiteTrack):
            if dsname is not None:
                if warnings:
                    # TODO: offer choice if more than 1 and hasGUI
                    warn("found more than one OrigamiSiteTrack module, will use the first one found")
            else:
                dsname = mod.outputDrift
                logger.debug("found drift with name '%s'" % dsname)
    if dsname is None:
...
class MINFLUXanalyser():
    def __init__(self, visFr):
        ...
        if has_comet:
            visFr.AddMenuItem('Corrections', "Comet drift correction", self.OnCometDriftCorrection)
            visFr.AddMenuItem('Corrections', "Plot comet drift correction", self.OnPlotCometDrift)
    ...
    def OnPlotCometDrift(self,event):
        ...
        if driftO is None:
            ...
            fig, axs = plt.subplots(2, 2)      
            fig.suptitle("comet drift estimate vs origami est (--)")
