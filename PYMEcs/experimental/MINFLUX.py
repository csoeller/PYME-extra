...
def findDriftComet(pipeline,warnings=True,return_mod=False,hasGUI=False):
    ...
    for mod in pipeline.recipe.modules:
        if isinstance(mod,DriftCorrComet):
            cmod = mod
            dsnames.append(mod.outputDrift)
            logger.debug("found drift with name '%s'" % dsnames[-1])
    if len(dsnames) < 1:
        if warnings:
            warn("we rely on drift info present in a datasource generated by the DriftCorrComet module.\n\n" +
                 "Can't find such a datasource, please add DriftCorrComet module to your recipe.\n\nAborting...")
        return None
        ...
        if hasGUI:
            ...
            with wx.SingleChoiceDialog(None, 'Comet drift', 'Select a datasource', options) as dlg:
                if dlg.ShowModal() != wx.ID_OK:
                    return None
                if dlg.GetSelection() < len(dsnames):
                    cdsname = dsnames[dlg.GetSelection()]
                    drift = {cdsname:pipeline.dataSources[cdsname]}
                else:
                    drift = {}
                    for dsname in dsnames:
                        drift[dsname] = pipeline.dataSources[dsname]
        else:
            warn("found more than one DriftCorrComet module, will use the first one found")
            drift = {cdsname:pipeline.dataSources[cdsname]}
    else:
        drift = {cdsname:pipeline.dataSources[cdsname]}
    if return_mod:
        ...
        return drift # note: drift is now a dict with possibly more than one driftcurve in it

def findDriftOrigami(pipeline,warnings=True,return_mod=False,warn_no_origami=False):
    from PYMEcs.recipes.localisations import OrigamiSiteTrack
    dsnames = []
    omod = None
    # search/check for instance
    for mod in pipeline.recipe.modules:
        if isinstance(mod,OrigamiSiteTrack):
            omod = mod
            dsnames.append(mod.outputDrift)
            logger.debug("found drift with name '%s'" % dsnames[-1])
    if len(dsnames) < 1:
        if warn_no_origami:
            warn("we rely on drift info present in a datasource generated by the OrigamiSiteTrack module.\n\n" +
                 "Can't find such a datasource, please add OrigamiSiteTrack module to your recipe.\n\nAborting...")
        return None
    if return_mod:
        return omod # we just return the last module found for now; TODO: possibly fix this behaviour!
    if len(dsnames) > 1:
        if warnings:
            # TODO: offer choice if more than 1 and hasGUI
            warn("found more than one OrigamiSiteTrack module, will add their contributions")
    drift = {'dsnames':dsnames.copy()}
    d0 = dsnames[0]
    for key in pipeline.dataSources[d0].keys():
        drift[key] = pipeline.dataSources[d0][key].copy() # we need to copy here so that the += below does not modify the original array!
    for i in range(1,len(dsnames)): # now cycle over remaining dsnames - if any
        ds = dsnames[i]
        d = pipeline.dataSources[ds]
        drift['x'] += d['x']
        drift['y'] += d['y']
        if 'z' in drift:
            drift['z'] += d['z']
...
class MINFLUXanalyser():
    ...
    def OnTrackMSDOverview(self, event):
        from PYMEcs.Analysis.Tracking import trackoverview
        pipeline = self.visFr.pipeline
        
        trackoverview(pipeline, use_clumpIndex=True, scaleDT=1.0) # for now no tweaking of scaleDT
    ...
    def OnOrigamiSiteRecipe(self, event=None):
        ...
        (preFiltered,corrSiteClumps,corrAll,preSiteClumps,
         ...
         sitesWithTracks,sites,sites_c,drift_estimate_sites) =  unique_names(['prefiltered','corrected_siteclumps',
                                                         'corrected_allpoints','preSiteclumps',
                                                         'siteclumps','dbscanClusteredSites',
                                                         'sitesWithSDs','sitesWithTracks',
                                                         'sites','sites_c','drift_estimate_sites'],pipeline.dataSources.keys())
        ...
        modules = [FilterTable(recipe,inputName=curds,outputName=preFiltered,
                   ...
                   OrigamiSiteTrack(recipe,inputClusters=siteClumps,inputSites=sites,outputName=corrSiteClumps,
                                    smoothingBinWidthsSeconds=400,outputDrift=drift_estimate_sites,
                                    outputAllPoints=corrAll,inputAllPoints=curds,labelKey='siteID',
                                    binnedStatistic='median'), # median to play it safe
