...
def findDriftComet(pipeline,warnings=True,return_mod=False):
    ...
    for mod in pipeline.recipe.modules:
        if isinstance(mod,DriftCorrComet):
            if dsname is not None:
                ...
                cmod = mod
                dsname = mod.outputDrift
                logger.debug("found drift with name '%s'" % dsname)
    if dsname is None:
        if warnings:
        ...
        return None
    drift = pipeline.dataSources[dsname]
    if return_mod:
        return cmod
    else:
        return drift
...
def findDriftOrigami(pipeline,warnings=True,return_mod=False):
    ...
    d0 = dsnames.pop(0) # pop the first dsname off the array
    for key in pipeline.dataSources[d0].keys():
        drift[key] = pipeline.dataSources[d0][key]
    for ds in dsnames: # now cycle over remaining dsnames - if any
...
class MINFLUXanalyser():
    ...
    def OnPlotCometDrift(self,event):
        pipeline = self.visFr.pipeline
        drift = findDriftComet(pipeline,warnings=True)
