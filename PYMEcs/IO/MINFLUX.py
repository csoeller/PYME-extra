...
# new option: add channel as option so we can look for module with specific channel name
def findmbm(pipeline,warnings=True,return_mod=False,channel=None):
    ...
    for mod in pipeline.recipe.modules:
        if isinstance(mod,MBMcorrection):
            if channel is None:
                dsname = mod.output
                break
            elif channel == mod.MBM_channel_name:
                dsname = mod.output
                break         
    if dsname is None:
        if warnings:
