...
def plot_stats_minflux(deltas, durations, tdintrace, efo_or_dtovertime, times,
                       ...
                       statStrings=None, timeStamp=None):
    ...
    if tdmedian < 50.0 and tdmedian >= 1.0:
        ndigs = 1
    elif tdmedian < 1.0:
        ndigs = 2
    else:
        ndigs = 0
    medstring = f"median {tdmedian:.{ndigs}f} ms"
    if tdmean < 50.0 and tdmean >= 1.0:
        ndigs = 1
    elif tdmean < 1.0:
        ndigs = 2
    else:
        ndigs = 0
    meanstring = f"mean {tdmean:.{ndigs}f} ms"
    ax2[0].set_xlabel('time between localisations in same trace [ms]')
    ax2[0].text(0.93, 0.8, medstring, horizontalalignment='right',
             verticalalignment='bottom', transform=ax2[0].transAxes)
    ax2[0].text(0.93, 0.7, meanstring, horizontalalignment='right',
             verticalalignment='bottom', transform=ax2[0].transAxes)
    ...
    if dsKey is not None:
        plt.suptitle('Series %s loc rates for DS "%s"' % (ts,dsKey))
    else:
        plt.suptitle('Series %s loc rates for unknown DS' % (ts))
    plt.tight_layout()
...
def analyse_locrate(data,datasource='Localizations',ds_coalesced='coalesced_nz',
    ...
    ts = data.mdh.get('MINFLUX.TimeStamp') # grep the timestamp from the datasource we analyzed
    ...
    if ds_coalesced in data.dataSources.keys():
        data.selectDataSource(ds_coalesced) # this could be wrong if more than 1 datasource!
    ...
    if plot:
        if showTimeAverages:
            ...
            plot_stats_minflux(deltas, durations_proper, tdintrace, delta_averages, delta_av_times,
                               ...
                               statStrings=stat_strings,timeStamp=ts)
        else:
            plot_stats_minflux(deltas, durations_proper, tdintrace, data['efo'], None, dsKey = datasource,
                               areaString=area_string, timeStamp=ts)

    return (starts,ends,deltas,durations_proper,tdintrace)

def find_shift_dsname(pipeline):
    from PYMEcs.recipes.localisations import AlignFromMBMs
    shiftnames = []
    # search/check for instance
    for mod in pipeline.recipe.modules:
        if isinstance(mod,AlignFromMBMs):
            shiftnames.append(mod.outputMBMshifts)
    if len(shiftnames) == 0:
        return None
    if len(shiftnames) > 1:
        import wx
        with wx.SingleChoiceDialog(None, 'Shift Selection', 'Select a shift datasource', shiftnames) as dlg:
            if dlg.ShowModal() != wx.ID_OK:
                return None
            keyidx = dlg.GetSelection()
        return shiftnames[keyidx]
    else:
        return shiftnames[0]

def plot_alignment_shifts(pipeline):
    # pipeline.dataSources['mbm_shifts']
    shiftname = find_shift_dsname(pipeline)
    if shiftname is None:
        warn("we rely on alignment info present in a datasource generated by the 'AlignFromMBMs' module.\n\n" +
                 "Can't find such a datasource, please first add the 'AlignFromMBMs' module to your recipe.\n\nAborting...")
        return

    sh = pipeline.dataSources[shiftname]
    shifts = sh.mdh.get('MINFLUX.MBMAlignmentShift')
    
    sdist = np.sqrt(sh['shift_x']**2+sh['shift_y']**2)
    doplot=(sdist < 50)*(sdist > 0)
    sx = sh['shift_x'][doplot]
    sy = sh['shift_y'][doplot]
    ...
    fig.suptitle(("x-y shift from MBM alignment %s" % shiftname) + valuestr)
